  ...
  const bigAddButtons = (
    <div className="flex flex-col gap-3">
      {catalogue.length > 0 && (
        <button type="button" onClick={() => setShowLibraryBrowser(true)}
          ...
          <span className="shrink-0 w-20 h-20 flex items-center justify-center">
            <RichIcon name="library" size={68} className="drop-shadow-[0_8px_14px_rgba(201,168,76,0.35)]"/>
          </span>
      ...
      <button type="button" onClick={add}
        ...
        <span className="shrink-0 w-20 h-20 flex items-center justify-center">
          <RichIcon name="notebookPen" size={68} className="drop-shadow-[0_8px_14px_rgba(0,0,0,0.22)]"/>
        </span>
        <span className="flex-1 text-left min-w-0">
